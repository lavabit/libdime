...
// old code for loading private key files generated by openssl tool, I'm
// changing this in order to read files we create programmatically via
// deserialization.
#if 0
/**
 * @brief
 *  Load an EC key from a file.
 * @param filename
 *  the name of the filename from which the key should be loaded.
 * @param gptr
 *  a pointer to an EC_GROUP variable that will receive the curve group on
 *  success.
 * @return
 *  a pointer to the loaded EC key on success, or NULL on failure.
 */
