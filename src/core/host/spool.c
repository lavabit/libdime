...
stringer_t * spool_path(int_t spool) {
	...
	if (core.spool) {
		result = st_merge_opts(NULLER_T | CONTIGUOUS | HEAP, "nnn", core.spool, (*(core.spool + ns_length_get(core.spool) - 1) == '/' ? "" : "/"), folder);
	}
	else {
...
void spool_stop(void) {
	...
	if (spool_files_cleaned - before > 0) {
		log_pedantic("%lu files needed to be purged from the spool.", spool_files_cleaned - before);
	}
