...
bool_t mm_sec_start(void) {
	...
	if ((secure.slab.length = (core.secure.memory.length + alignment - 1) & ~(alignment - 1)) < MM_SEC_POOL_LENGTH_MIN) {
	...
	secure.slab.length_true = secure.slab.length + (core.page_length * 2);
